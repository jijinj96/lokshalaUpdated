	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try 
		{
			...
			else
			{
				...
				ArrayList<ApplicantDetails> applicant = account.getApplicants(email);	
				if(applicant == null){
					obj.put("msg", "no applicants");
				}
				else{
					JSONArray application = new JSONArray();
					for(ApplicantDetails a : applicant){
						JSONObject app = new JSONObject();
						app.put("name", a.getName());
						app.put("child-name", a.getChild_name());
						app.put("contact", a.getContact());
						app.put("id", a.getId());
						application.add(app);
					}
